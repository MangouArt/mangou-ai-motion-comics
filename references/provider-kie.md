...
- 视频：`bytedance/seedance-2-fast`；Mangou NewAPI Agent Gateway 的 KIE 官方计价模型可使用 `doubao-seedance-2-0-fast-260128`。
...
- Mangou NewAPI Agent Gateway 实测 `doubao-seedance-2-0-fast-260128` 720p/5s 视频必须传 `params.quality: "720p"`；缺失会报 `Video quality cannot be empty`，`quality: "standard"` 会命中未配置的定价层。
